...
export class ProtocolManager implements IProtocolManager {
    ...
    public initialize(): void {
        ...
        // In Sidecar mode, protocol handling is done by Tauri process
        if (!('sendTauriRequest' in this.app)) {
            ...
            this.app.logger.info('ProtocolManager initialized in Sidecar mode - protocols handled by Tauri');
        }
    }

    private startHandling(): void {
        // Only setup protocol handler in Electron mode
        if ('sendTauriRequest' in this.app) {
            return; // In Sidecar mode, protocols are handled by Tauri
        }

        const protocol = require('electron').protocol;

        // Setup protocol handler
        protocol.handle(AppProtocol, async (request: any) => {
