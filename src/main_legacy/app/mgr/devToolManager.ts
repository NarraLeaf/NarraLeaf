...
import { App, AppMeta } from "@/main_legacy/app/app";
...
export class DevToolManager {
    ...
    constructor(private app: App | SidecarService) {
    }
    ...
    private setupDevServerHandlers(): void {
        ...
        this.wsClient.onMessage(DevServerEvent.RequestMainQuit, () => {
            this.wsClient?.close();
            // In Sidecar mode, request shutdown through the service
            if ('onShutdown' in this.app) {
                this.app.onShutdown(() => {});
            } else {
                // Fallback for legacy Electron mode
                require('electron').app.quit();
            }
        });

        this.wsClient.onMessage(DevServerEvent.RequestPageRefresh, () => {
            // In Sidecar mode, send request to Tauri process to refresh window
            if ('sendTauriRequest' in this.app) {
                this.app['sendTauriRequest']('window.refresh', {}).catch(error => {
    ...
    private setupDevUserData(): void {
        // In Sidecar mode, userData is handled by Tauri
        if (!('sendTauriRequest' in this.app)) {
            const electronApp = require('electron').app;
            electronApp.setPath("userData", path.join(this.app.getAppPath(), "userData-dev"));
        }
    }
