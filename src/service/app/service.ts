...
export class Service {
    ...
    public async prepare() {
        ...
        this.logger.info("[Service] Preparing service and establishing IPC connection...");

        while (retries > 0 && !connected) {
            try {
                const attemptNum = maxRetries - retries + 1;
                this.logger.info(`[Service] Attempting to connect to IPC server (${attemptNum}/${maxRetries})...`);
                await this.ipcClient.connect();
                connected = true;
                this.logger.info("[Service] Successfully connected to IPC server");
            } catch (error) {
                retries--;
                ...
                if (isServerNotReady && retries > 0) {
                    ...
                    this.logger.info(`[Service] IPC server not ready yet, waiting ${Math.round(delay)}ms before retry (${retries} retries left)...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                } else if (retries > 0) {
                    // For other errors, use longer delays
                    const delay = Math.min(1000 * (maxRetries - retries + 1), 5000);
                    this.logger.warn(`[Service] IPC connection failed: ${errorMsg}. Waiting ${delay}ms before retry (${retries} retries left)...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                } else {
                    this.logger.error(`[Service] Failed to connect to IPC server after ${maxRetries} attempts: ${errorMsg}`);
                    throw error;
                }
            }
        }
